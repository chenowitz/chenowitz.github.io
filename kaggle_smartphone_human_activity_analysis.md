...
The best accuracy I was able to achieve was 96.37% with a linear kernel and a C value of ~0.0623.
...
I also used Optuna to conduct a search for the best hyperparameters for a simple neural network with one hidden layer. 

I specified a search with:
Learning rate: Between 1e-5 and 1e-1
Optimizer: Adam, RMSprop, or SGD
Activation function: ReLU or Tanh
Number of hidden layers: Between 1 and 6

The maximum accuracy achieved was ~95.66% using Tanh as the activiation function, a learning rate of 0.0025, 6 neurons in the hidden layer and SGD as the optimizer.

The windowing and data processing techniques used by the researchers to produce this dataset are typically applied prior to the use of linear classification techniques.
It would be interesting to see if a neural network would have yielded better results on the raw dataset than it did on this preprocessed one.
